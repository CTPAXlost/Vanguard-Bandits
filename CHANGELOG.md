# 1.9.5 — Восстановление запуска боя и миссии VI

- Исправлена реальная причина незагрузки миссий I–V: `super._ready()` в контроллере главы VI теперь вызывается для каждой миссии, а не только при `current_mission == 6`.
- Миссия VI создаёт управляемый отряд из Bastion, Andrew, Zeira, Ione и Reyna до начала первого хода.
- Logan, Claire и Shion изначально принадлежат Южному королевству; Alden, Devlin и Barlow — Северному. Выбор игрока корректно превращает выбранную сторону в союзников, а вторую — во врагов.
- Runtime-smoke больше не вручную собирает бой и не скрывает ошибки производственной инициализации.
- Добавлена обязательная проверка нормального запуска миссий I–VI.
- Магазин выводит полный каталог из 12 предметов в стабильном порядке и проверяет наличие всех иконок.
- GitHub Actions снова строгий: убраны все `continue-on-error`; импорт, runtime-smoke и Windows-export обязательны.
- Сохранены полностью открытые проходы замка, позиции Sadira/Franco/Halak, защита Zakov от случайного удара по своим, атака Faulkner «Град огня с неба» и весь контент войны Севера и Юга.
