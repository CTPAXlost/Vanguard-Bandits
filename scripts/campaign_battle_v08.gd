...
func _resolve_attack(attacker: Node3D, target: Node3D, mode: String) -> void:
	...
	if miss_chance > 0.0 and rng.randf() <= miss_chance:
		status_label.text = "%s промахивается!" % str(attacker.get_meta("label"))
	...
	var unblockable: bool = bool(attack_data.get("unblockable", false))
	...
	if not unblockable:
		...
		status_label.text = "От «%s» нельзя защититься или уклониться." % str(attack_data.get("label", mode))
	var multiplier: float = float(attack_data.get("multiplier", 1.0))
	if multiplier > 0.0:
		var damage: int = _calculate_damage(attacker, target, multiplier)
		if reaction == "defend" and not back_attack and not unblockable:
	...
	if mode == "desert_whirl" and _is_alive(target):
		...
		status_label.text = "%s дезориентирован и может атаковать своего." % str(target.get_meta("label"))
	await _apply_generic_attack_effects(attacker, target, mode, attack_data)
	if mode == "slide":
		await _complete_slide_pass_through(attacker, target)
	if reaction in ["counter_slash", "counter_lunge"] and _is_alive(target) and not back_attack and not unblockable:
...
func _apply_generic_attack_effects(attacker: Node3D, target: Node3D, mode: String, attack_data: Dictionary) -> void:
	if target == null or not is_instance_valid(target):
		return
	if _is_alive(target) and attack_data.has("freeze_chance") and mode != "ice_rain":
		if rng.randf() <= float(attack_data.get("freeze_chance", 0.0)):
			var freeze_turns: int = int(attack_data.get("freeze_turns", 1))
	...
	elif _is_alive(target) and mode == "ice_rain":
		if rng.randf() <= float(attack_data.get("freeze_chance", 0.30)):
			var ice_turns: int = int(attack_data.get("freeze_turns", 2))
			target.set_meta("frozen_turns", maxi(int(target.get_meta("frozen_turns", 0)), ice_turns))
			status_label.text = "%s заморожен на %d хода." % [str(target.get_meta("label")), ice_turns]
			_spawn_ice_lock_effect(target.global_position + Vector3(0, 1.0, 0))
	if _is_alive(target) and attack_data.has("blind_chance") and rng.randf() <= float(attack_data.get("blind_chance", 0.0)):
		var blind_turns: int = int(attack_data.get("blind_turns", 1))
		target.set_meta("blind_turns", maxi(int(target.get_meta("blind_turns", 0)), blind_turns))
		target.set_meta("miss_chance", 1.0)
		status_label.text = "%s ослеплён на %d хода." % [str(target.get_meta("label")), blind_turns]
	if _is_alive(target) and attack_data.has("disable_chance") and mode != "guillotine" and rng.randf() <= float(attack_data.get("disable_chance", 0.0)):
		var disable_turns: int = int(attack_data.get("disable_turns", 1))
		target.set_meta("disabled_turns", maxi(int(target.get_meta("disabled_turns", 0)), disable_turns))
		status_label.text = "%s выведен из строя на %d хода." % [str(target.get_meta("label")), disable_turns]
	if _is_alive(target) and attack_data.has("hypnosis_chance") and rng.randf() <= float(attack_data.get("hypnosis_chance", 0.0)):
		var hypnosis_turns: int = int(attack_data.get("hypnosis_turns", 3))
		target.set_meta("disoriented_turns", hypnosis_turns)
		target.set_meta("friendly_fire_chance", 1.0)
		status_label.text = "%s под гипнозом и будет атаковать союзников %d хода." % [str(target.get_meta("label")), hypnosis_turns]
	if _is_alive(target) and attack_data.has("domination_chance") and rng.randf() <= float(attack_data.get("domination_chance", 0.0)):
		target.set_meta("original_team", str(target.get_meta("team", "enemy")))
		target.set_meta("team", str(attacker.get_meta("team", "ally")))
		target.set_meta("player", false)
		target.set_meta("dominated", true)
		status_label.text = "%s подчинён до конца миссии." % str(target.get_meta("label"))
	if _is_alive(target) and int(attack_data.get("bleed_damage", 0)) > 0:
		target.set_meta("bleed_damage", int(attack_data.get("bleed_damage", 0)))
		status_label.text = "%s получает кровотечение: −%d HP каждый ход." % [str(target.get_meta("label")), int(attack_data.get("bleed_damage", 0))]
	var area_targets: int = int(attack_data.get("area_targets", 1))
	if area_targets > 1 and mode != "sticky_sandstorm" and _is_alive(target):
		var secondary_ratio: float = float(attack_data.get("secondary_ratio", 0.50))
		var affected: int = 1
		for candidate: Node3D in units:
			if affected >= area_targets:
				break
			...
			if _grid_distance(target, candidate) > 2:
				continue
			var secondary_damage: int = maxi(1, int(float(_calculate_damage(attacker, candidate, float(attack_data.get("multiplier", 1.0)))) * secondary_ratio))
			await _damage_target(candidate, secondary_damage)
			affected += 1
	var splash_ratio: float = float(attack_data.get("splash_ratio", 0.0))
	if splash_ratio > 0.0 and _is_alive(target):
		for nearby: Node3D in units:
			if nearby == target or not _is_alive(nearby):
				continue
			if str(nearby.get_meta("team", "")) != str(target.get_meta("team", "")) or _grid_distance(target, nearby) > 1:
				continue
			await _damage_target(nearby, maxi(1, int(float(_calculate_damage(attacker, nearby, float(attack_data.get("multiplier", 1.0)))) * splash_ratio)))
...
func _choose_ai_attack(unit: Node3D, target: Node3D) -> String:
	...
	for preferred: String in ["slide", "ice_age", "hyper_blizzard", "shred", "honor_strike", "fate_strike", "shining_sky", "lord_dark_strike", "hell_gate", "heavy_rain", "storm_vortex", "devlin_combo", "area_rocket", "geno_flame", "earthquake_area", "northern_lights", "ninja_blade", "energy_wheel", "ice_rain", "ultrasound", "spear_throw", "quicksand", "desert_whirl", "bright_bomb", "earthquake", "tornado", "strong_slash", "ball_lightning", "long_lunge", "lunge", "slash"]:
		if not modes.has(preferred):
			continue
		if _ai_attack_is_usable(unit, preferred, distance):
			return preferred
	var best_mode: String = ""
	var best_score: float = -1.0
	for candidate: String in modes:
		if not _ai_attack_is_usable(unit, candidate, distance):
			continue
		var candidate_data: Dictionary = CombatCatalog.attack(candidate)
		var score: float = float(candidate_data.get("multiplier", 0.0)) + float(candidate_data.get("range", 1)) * 0.02
		if score > best_score:
			best_score = score
			best_mode = candidate
	return best_mode if not best_mode.is_empty() else "slash"


func _ai_attack_is_usable(unit: Node3D, mode: String, distance: int) -> bool:
	var data: Dictionary = CombatCatalog.attack(mode)
	...
	if range_mode == "up_to":
		...
		return false
	var costs: Dictionary = CombatCatalog.resource_cost(mode)
	return _can_spend_fatigue(unit, int(costs.get("fatigue", 0))) and _can_spend_energy(unit, int(costs.get("energy", 0)))
...
func _play_attack_animation(attacker: Node3D, target: Node3D, mode: String) -> void:
	var animation_id: String = str(CombatCatalog.attack(mode).get("animation", mode))
	match animation_id:
		"lunge":
...
func _tick_status_effects(unit: Node3D) -> void:
	var slow_turns: int = int(unit.get_meta("slow_turns", 0))
	if slow_turns > 0:
		slow_turns -= 1
		unit.set_meta("slow_turns", slow_turns)
		if slow_turns <= 0:
			unit.set_meta("slow_move_limit", 0)
			if int(unit.get_meta("blind_turns", 0)) <= 0:
				unit.set_meta("miss_chance", 0.0)
	var blind_turns: int = int(unit.get_meta("blind_turns", 0))
	if blind_turns > 0:
		blind_turns -= 1
		unit.set_meta("blind_turns", blind_turns)
		if blind_turns <= 0 and int(unit.get_meta("slow_turns", 0)) <= 0:
			unit.set_meta("miss_chance", 0.0)
	var bleed_damage: int = int(unit.get_meta("bleed_damage", 0))
	if bleed_damage > 0 and _is_alive(unit):
		var stats: Dictionary = _stats(unit)
		stats["hp"] = maxi(0, int(stats.get("hp", 0)) - bleed_damage)
		unit.set_meta("stats", stats)
		_refresh_hp_bar(unit)
		_spawn_damage_label(unit.global_position + Vector3(0, 2.35, 0), bleed_damage)
		status_label.text = "%s теряет %d HP от кровотечения." % [str(unit.get_meta("label")), bleed_damage]
		if int(stats.get("hp", 0)) <= 0:
			call_deferred("_finish_status_defeat", unit)


func _finish_status_defeat(unit: Node3D) -> void:
	if unit != null and is_instance_valid(unit) and int(_stats(unit).get("hp", 0)) <= 0:
		await _damage_target(unit, 0)
