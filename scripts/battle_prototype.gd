...
func _refresh_ui() -> void:
	...
	var magic_summary := CombatCatalog.magic_summary_for_unit(selected_unit)
	var ability_summary := CombatCatalog.ability_summary_for_unit(selected_unit)
	var maximum_atac_level: int = CombatCatalog.max_level_for_unit(selected_unit)
	var next_unlock_summary: String = CombatCatalog.next_unlock_summary_for_unit(selected_unit)
	stats_label.text = (
		(
			"Уровень ATAC: %d / %d\nСила: %d (+ оружие %d)\nЛовкость: %d\n"
			+ "Защита: %d\nМагия: %s\nУмение: %s\nХод: %d клеток"
	...
	equipment_label.text = (
		"Следующий приём: %s\nОружие: %s\nСнаряжение: %s\nАмулет: %s"
		% [next_unlock_summary, str(stats["weapon"]), str(stats["equipment"]), str(stats["amulet"])]
	)
